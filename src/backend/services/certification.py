...
class CertificationService:
    def __init__(self, repo: CertificationRepository, audit_repo: AuditRepository) -> None:
        self.repo = repo
        self.audit_repo = audit_repo

    async def list(self, candidate_id: UUID) -> list[Certification]:
        return await self.repo.list_ordered(candidate_id)

    async def get(self, candidate_id: UUID, cert_id: UUID) -> Certification:
    ...
    async def update(
        ...
        cert = await self.get(candidate_id, cert_id)
