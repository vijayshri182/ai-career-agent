...
class EducationService:
    ...
    async def list(self, candidate_id: UUID, user_id: UUID) -> list[Education]:
        await self._assert_candidate_owned(candidate_id, user_id)
        return await self.repo.list_ordered(candidate_id)

    async def get(
        self, candidate_id: UUID, user_id: UUID, education_id: UUID
    ) -> Education:
        await self._assert_candidate_owned(candidate_id, user_id)
    ...
    async def update(
        ...
        edu = await self.get(candidate_id, user_id, education_id)
