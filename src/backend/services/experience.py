...
class ExperienceService:
    def __init__(self, repo: ExperienceRepository, audit_repo: AuditRepository) -> None:
        self.repo = repo
        self.audit_repo = audit_repo

    async def list(self, candidate_id: UUID) -> list[Experience]:
        return await self.repo.list_ordered(candidate_id)

    async def get(self, candidate_id: UUID, experience_id: UUID) -> Experience:
    ...
    async def update(
        ...
        exp = await self.get(candidate_id, experience_id)
