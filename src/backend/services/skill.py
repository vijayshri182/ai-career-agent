...
class SkillService:
    def __init__(self, repo: SkillRepository, audit_repo: AuditRepository) -> None:
        self.repo = repo
        self.audit_repo = audit_repo

    async def list(self, candidate_id: UUID) -> list[CandidateSkill]:
        return await self.repo.list(candidate_id=candidate_id)

    async def get(self, candidate_id: UUID, skill_id: UUID) -> CandidateSkill:
    ...
    async def update(
        ...
        skill = await self.get(candidate_id, skill_id)
